...
import app.badlogicgames.superjumper.Animation;
...
public class WorldRenderer {
	...
	public void render () {
		if (world.local_bob.position.y > cam.position.y) cam.position.y = world.local_bob.position.y;
		cam.update();
	...
	private void renderBob () {
		...
		switch (world.local_bob.state) {
		case Bob.BOB_STATE_FALL:
			keyFrame = Assets.bobFall.getKeyFrame(world.local_bob.stateTime, Animation.ANIMATION_LOOPING);
			break;
		case Bob.BOB_STATE_JUMP:
			keyFrame = Assets.bobJump.getKeyFrame(world.local_bob.stateTime, Animation.ANIMATION_LOOPING);
			break;
		...
		float side = world.local_bob.velocity.x < 0 ? -1 : 1;
		if (side < 0){
	...
	private void renderEnemy(){
		batch.draw(Assets.enemyBob, _x, _y, _width, _height);
	}
	...
	public void updateEnemyLocation(float x, float y, float width, float height) {
		this._x = x;
		this._y = y;
		this._width = width;
		this._height = height;
